...
const Employeeinfodata = () => {
  ...
  const handle_fetchCut_Lay_Prod_Search_Grid_Data = async (
    ...
    try {
      ...
      const endpoint =
        'http://192.168.15.15:30/api/Laycutting/Cut_Lay_Prod_Search_Grid/';
      const requestData = {
        param1: buyerID || 0,
        param2: 0,
        param3: 0,
      ...
      // API Call
      const response = await instance.post(endpoint, requestData, config);
  ...
  const GetDropDownBuyer = async () => {
    try {
      const response = await instance.post(
        'http://192.168.15.15:30/api/Common/GetDropDownBuyer',
        {},
        {
          headers: {
            Authorization: `Bearer ${gettoken?.token}`,
  ...
  const GetFloor = async () => {
    try {
      const response = await instance.post(
        'http://192.168.15.15:30/api/Common/get_floor_id',
        {},
        {
          headers: {
            Authorization: `Bearer ${gettoken?.token}`,
  ...
  const handelChangeCuttinglawValue = (id: string) => {
    console.log('id' + id);
  };
  ...
  return (
    <View style={{flex: 1, flexDirection: flexD, backgroundColor: '#fff'}}>
      ...
      <View
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <Pressable onPress={() => handelChangeCuttinglawValue(item?.cutno)}>
      ...
      {inputfiledready && <Text style={{textAlign: 'center'}}>Scan Now</Text>}
